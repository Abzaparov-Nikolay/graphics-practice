...
namespace Practice7
{
    public class MeshBuilder
    {
        ...
        public static (List<Vector3>, List<int>, List<Vector2>) BuildWithTexture(
            ...
            Vector3[] capRotations)
        {
            ...
            var texCoords = CreateTextureCoords(sides);
            ...
            for (var cap = 0; cap < 2; cap++)
            {
                ...
                if ((cap == 0 || cap == 2 - 1) && !isFirstLastConnected) // drawing
                {
        ...
        private static List<Vector2> CreateTextureCoords(int sides)
        {
            var texCoords = new List<Vector2>();
            var angle = MathHelper.Pi / sides;
            foreach (var (yMult, yStart) in new (int, int)[] { (-1, 1), (1, 0) })
            {
                for (var i = 0; i < sides; i++)
                {
                    texCoords.Add(new Vector2(0.5f + (float)MathHelper.Cos(angle * i),
                        yStart + 0.25f * yMult * (float)MathHelper.Sin(angle * i)));
                }
            }
            return texCoords;
        }

        private static List<Vector2> CreateTextureCoords(int caps, int sides, bool isSphere)
        {
            var texCoords = new List<Vector2>();

            float xPiMult = 2f - 2f / (sides + 1);
            float yPiMult = 2f -2f / (caps + 1);

            float xAngle = xPiMult * MathHelper.Pi / sides;
            float yAngle = yPiMult * MathHelper.Pi / caps;

            for (var cap = 0; cap < caps; cap++)
            {
                for (var side = 0; side < sides; side++)
                {
                    if (isSphere && (cap == 0 || cap == caps - 1) && side == 0)
                    {
                        texCoords.Add(new Vector2(0.5f , 0.5f +0.5f* (float)MathHelper.Sin(yAngle * cap)));
                    }
                    else
                    {
                        texCoords.Add(new Vector2(0.5f + 0.5f * (float)MathHelper.Cos(xAngle * side), 0.5f + 0.5f * (float)MathHelper.Sin(yAngle * cap)));
                    }
                }
            }
            return texCoords;
        ...
        public static (List<Vector3>, List<int>, List<Vector2>) Build(
            int caps,
            int sides,
            float radius,
            float[] capScales,
            ...
            Vector3[] capRotations)
        {
            var vertices = new List<Vector3>();
            var indices = new List<int>();
            var texCoords = CreateTextureCoords(caps, sides, false);
        ...
        public static (Vector3[], int[], Vector2[]) BuildSphere(int sides, float radius, int capSides)
        {
            var vertices = new List<Vector3>();
            var indices = new List<int>();
            var texCoords = CreateTextureCoords(sides, capSides, true);
            ...
            for (var side = 0; side < sides; side++)
            {
                ...
                {
                    indices.AddRange(ConnectCaps(Enumerable.Range(2 + side * (capSides - 2), capSides - 2).ToArray(),
                ...
                {
                    indices.AddRange(ConnectCaps(Enumerable.Range(2, capSides - 2).ToArray(),
            ...
            List<int> ConnectCaps(int[] first, int[] second)
            {
                var indicesList = new List<int>();
